#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time : 2019/12/29 21:28
# @Author : Micky
# @Desc : 右眼关键点模型训练
# @File : model_right_eye_net.py
# @Software: PyCharm
...
class RNet:
    ...
    def _buildGraph(self):
        ...
        """
        # 定义占位符
        input_x = tf.placeholder(tf.float32, shape=[None, self.width, self.height, self.channel], name='input_x_r')
        input_y = tf.placeholder(tf.float32, shape=[None, self.n_classify], name='input_y_r')
        self.x = input_x
        self.y = input_y
        self.training = tf.placeholder(tf.bool, name='training_r')
        self.keepdrop = tf.placeholder(tf.float32, name='keep_drop_r')
        self.lr = tf.placeholder(tf.float32, name='learning_rate_r')

        # 执行卷积过程
        with tf.variable_scope('conv1_r'):
        ...
        with tf.variable_scope('conv2_r'):
        ...
        with tf.variable_scope('conv3_r'):
            # 高宽减半了。
        ...
        with tf.variable_scope('conv4_r'):
    ...
    def train(self, model_save, base_dir, display):
        ...
        :return:
        """
        file_path = os.path.join(base_dir, 'tfrecord/right_eye.tfrecord_shuffle')
        # 读取tfrecord文件
        ...
        label_file = os.path.join(base_dir, 'righteyes.txt')
    ...
    def read_single_tfrecord(self, tfrecord_file):
        ...
        image_features = tf.parse_single_example(serialized_example,
                                                 features={
                                                     ...
                                                     'image/label': tf.FixedLenFeature([12], tf.float32)
                                                 })
        ...
        label = tf.reshape(label, [ConfigN.BOX_BATCH_SIZE, 12])
        return image, label
