#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time : 2019/12/27 11:04
# @Author : Micky
# @Desc : 预测眼睛、鼻子、嘴巴区域的网络模型
# @File : model_four_area.py
# @Software: PyCharm
...
class FNet(object):
    ...
    def _buildGraph(self):
        ...
        input_center = tf.placeholder(tf.float32, shape=[None, 8], name='input_center')
        self.x = input_x
        self.y = input_y
        ...
        with tf.variable_scope('out2'):
            logits_2 = tf.layers.dense(fc1, 8)
            # prob = tf.nn.softmax(logits)

        # 计算损失 欧几里得距离
        loss1= tf.losses.mean_squared_error(logits, input_y)
        # loss1 = tf.sqrt(tf.reduce_sum(tf.square(logits - input_y)))
        loss2 = tf.sqrt(tf.reduce_sum(tf.square(logits_2 - input_center)))
        ...
        # 添加标量统计结果
        tf.summary.scalar("bbox_loss", loss)
    ...
    def train(self, model_save, base_dir, display):
        ...
        :return:
        """
        file_path = os.path.join(base_dir, 'tfrecord/four_area.tfrecord_shuffle')
        # 读取tfrecord文件
        image_batch, label_batch, center_batch = self.read_single_tfrecord(file_path)
        # 模型图存储文件夹
        logs_dir = '../graph/FNet/'
        # 如果不存在则创建文件夹
        ...
        label_file = os.path.join(base_dir, 'image.txt')
        ...
        try:
            for step in range(MAX_STEP):
                ...
                if coord.should_stop():
                    break
                image_batch_array, label_batch_array, center_batch_array = self.sess.run(
                    [image_batch, label_batch, center_batch])
                _, summary = self.sess.run([self.opt_train, self.summary_op],
                                           feed_dict={self.x: image_batch_array,
                                                      self.y: label_batch_array,
                                                      self.center: center_batch_array,
                                                      self.keepdrop: 0.5,
                                                      self.lr: 0.0001,
                                                      self.training: True})
                # 展示训练过程
    ...
    def read_single_tfrecord(self, tfrecord_file):
        ...
        image_features = tf.parse_single_example(serialized_example,
                                                 features={
                                                     ...
                                                     'image/label': tf.FixedLenFeature([16], tf.float32),
                                                     'image/center': tf.FixedLenFeature([8], tf.float32)
                                                 })
        ...
        center = tf.cast(image_features['image/center'], tf.float32)
        image, label, center = tf.train.batch([image, label, center], batch_size = ConfigN.BOX_BATCH_SIZE,
                                                     num_threads=2,
                                                     capacity = ConfigN.BOX_BATCH_SIZE)
        label = tf.reshape(label, [ConfigN.BOX_BATCH_SIZE, 16])
        center = tf.reshape(center, [ConfigN.BOX_BATCH_SIZE, 8])
    ...
    def predict(self, X):
        ...
        prob, point = self.sess.run([self.logits, self.logits_center], feed_dict=feed_dict)
        return prob, point
    ...
    def probability(self, X):
        ...
        """
        X = np.asarray(X)
        # landmark/models3/best_model.ckpt.meta
        checkpoint = current_path + '/model/best_model.ckpt'
        # 模型恢复
